...
resource "aws_security_group" "backend_sg" {
  name        = "backend-sg"
  description = "Acceso restringido para Spring Boot desde el Frontend"
  vpc_id      = aws_vpc.innovatech_vpc.id

  # CORRECCIÓN: Puerto 8080 abierto para las APIs de Java Spring Boot
  ingress {
    from_port       = 8080
    to_port         = 8080
    protocol        = "tcp"
...
# ==========================================
# 3. SCRIPT DE INICIALIZACIÓN (DOCKER AUTOMÁTICO)
# ==========================================
...
locals {
  docker_script = <<-EOF
    ...
    # CORRECCIÓN: Se cambió "sources.list.p" por "sources.list.d"
    ...
    # OPCIONAL/RECOMENDADO: Aquí puedes añadir las líneas para arrancar tus contenedores automáticamente, por ejemplo:
    # docker run -d -p 80:80 seba35/innovatech-front:latest
  EOF
}
